...
void input()
{
	...
	for (int i = 0; i < K; i++)
	{
		int r, c, s;
		cin >> r >> c >> s;
		kv.push_back({ r - 1, c - 1, s }); //인덱스 맞추기
	}
}
...
void Go(int r, int c, int s)
{
	for (int k = 1; k < s + 1; k++)
	{
		...
		int temp = Array[r - k][c - k];
		for (int i = 0; i < n; i++)
			Array[r - k + i][c - k] = Array[r - k + i + 1][c - k];

		for (int i = 0; i < n; i++)
			Array[r + k][c - k + i] = Array[r + k][c - k + i + 1];
